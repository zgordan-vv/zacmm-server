.PHONY: build package run stop run-client run-server stop-client stop-server restart restart-server restart-client start-docker clean-dist clean nuke check-style check-client-style check-server-style check-unit-tests test dist prepare-enteprise run-client-tests setup-client-tests cleanup-run-client-tests test-client build-linux build-osx build-windows internal-test-web-client vet run-server-for-web-client-tests diff-config prepackaged-plugins prepackaged-binaries test-server test-server-ee test-server-quick test-server-race start-docker-check
...
GO=go
DELVE=dlv
LDFLAGS += -X "github.com/zgordan-vv/zacmm-server/model.BuildNumber=$(BUILD_NUMBER)"
LDFLAGS += -X "github.com/zgordan-vv/zacmm-server/model.BuildDate=$(BUILD_DATE)"
LDFLAGS += -X "github.com/zgordan-vv/zacmm-server/model.BuildHash=$(BUILD_HASH)"
LDFLAGS += -X "github.com/zgordan-vv/zacmm-server/model.BuildHashEnterprise=$(BUILD_HASH_ENTERPRISE)"
LDFLAGS += -X "github.com/zgordan-vv/zacmm-server/model.BuildEnterpriseReady=$(BUILD_ENTERPRISE_READY)"
...
run-server: prepackaged-binaries validate-go-version ## Starts the server.
...
debug-server: start-docker ## Compile and start server using delve.
	...
	$(DELVE) debug $(PLATFORM_FILES) --build-flags="-ldflags '\
		-X github.com/zgordan-vv/zacmm-server/model.BuildNumber=$(BUILD_NUMBER)\
		-X \"github.com/zgordan-vv/zacmm-server/model.BuildDate=$(BUILD_DATE)\"\
		-X github.com/zgordan-vv/zacmm-server/model.BuildHash=$(BUILD_HASH)\
		-X github.com/zgordan-vv/zacmm-server/model.BuildHashEnterprise=$(BUILD_HASH_ENTERPRISE)\
		-X github.com/zgordan-vv/zacmm-server/model.BuildEnterpriseReady=$(BUILD_ENTERPRISE_READY)'"
